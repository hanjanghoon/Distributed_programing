 ...
 * 
 * Compile:  gcc -g -Wall -o pth_ll_rwl_edited pth_ll_rwl_edited.c 
 *              my_rand.c -lpthread
...
#  endif
    ...
    //pthread_cond_init(&reader_turn, NULL);
...
#  endif
   ...
   // pthread_cond_destroy(&reader_turn);
...
void* Thread_work(void* rank) {
    ...
    for (i = 0; i < ops_per_thread; i++) {
        ...
        if (which_op < search_percent) {
            ...
            while(writers != 0)  pthread_cond_wait(&turn, &mutex);
            readers += 1;
            ...
            pthread_mutex_lock(&mutex);
            readers -= 1;
            if(readers == 0)        pthread_cond_signal(&turn);
            pthread_mutex_unlock(&mutex);
            ...
            pthread_mutex_lock(&mutex);
            while((readers != 0) || (writers != 0)) pthread_cond_wait(&turn, &mutex);
            writers +=  1;
            pthread_mutex_unlock(&mutex);
           ...
           // pthread_cond_broadcast(&reader_turn);
            pthread_mutex_unlock(&mutex);
